...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE (
      (np.professional_email = NEW.rater_user_email AND n.user_email = NEW.rated_user_email)
      OR
      (n.user_email = NEW.rater_user_email AND np.professional_email = NEW.rated_user_email)
    )
